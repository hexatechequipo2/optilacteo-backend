import { Body, Controller, Get, Put, Post, UseGuards, HttpCode, HttpStatus } from '@nestjs/common';
...
export class PlcConfigController {
  ...
  @ApiOperation({ summary: 'Obtener configuración de URL del PLC de la empresa' })
  ...
  @ApiResponse({ status: 403, description: 'No tiene permisos para modificar esta configuración' })
  guardarUrl(
  ...
  @Permissions(ModuloSistema.SENSORES_IOT, 'canWrite')
  @ApiOperation({ summary: 'Probar conexión con una URL de PLC antes de guardarla' })
  @ApiResponse({ status: 200, description: 'Resultado del test (ok true/false)' })
  @ApiResponse({ status: 400, description: 'URL con formato inválido' })
