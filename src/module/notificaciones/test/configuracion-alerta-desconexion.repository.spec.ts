...
describe('ConfiguracionAlertaDesconexionRepository', () => {
  ...
  let typeOrmRepository: jest.Mocked<Repository<ConfiguracionAlertaDesconexion>>;
  ...
  it('findByEmpresa: debe buscar por empresaId', async () => {
    const mockData = { id: 1, empresaId: 10, umbralMinutos: 15 } as ConfiguracionAlertaDesconexion;
    typeOrmRepository.findOne.mockResolvedValue(mockData);

    const result = await repository.findByEmpresa(10);

    expect(typeOrmRepository.findOne).toHaveBeenCalledWith({ where: { empresaId: 10 } });
    expect(result).toEqual(mockData);
  });

  it('save: debe guardar la entidad', async () => {
    const mockData = { empresaId: 10, umbralMinutos: 15 } as ConfiguracionAlertaDesconexion;
    typeOrmRepository.save.mockResolvedValue({ ...mockData, id: 1 } as any);
